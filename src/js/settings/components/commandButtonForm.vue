...
<template>
    <n-card>
        <n-form ref="formRef" :label-width="80" :model="node" size="small">
            <n-form-item :label="$t('settings.commandButtonForm.name')" path="label">
                <n-input v-model:value="node.label" placeholder="" />
            </n-form-item>
            <n-form-item :label="$t('settings.commandButtonForm.icon')" path="icon">
                <div style="display: flex;width: 100%;align-items: center;">
                    ...
                    <n-button round style="margin-left:8px"
                        @click="node.icon = mdiIconList[Math.floor(Math.random() * mdiIconList.length)].value">{{ $t('settings.commandButtonForm.random') }}</n-button>
                </div>
            </n-form-item>
            <n-form-item :label="$t('settings.commandButtonForm.command')" path="command">
                <div style="height: 200px;width: 100%;">
                    <code-editor v-model="node.command" language="shell"></code-editor>
                </div>
            </n-form-item>
        </n-form>
        <template #footer>
            <div style="width:100%;display: flex;justify-content: flex-end;">
                <n-button round size="small" @click="onCanceled">{{ $t('settings.commandButtonForm.cancel') }}</n-button>
                <n-button style="margin-left: 8px;" type="primary" round size="small" @click="onSubmitted">{{ $t('settings.commandButtonForm.confirm') }}</n-button>
            </div>
        </template>
    </n-card>
</template>
